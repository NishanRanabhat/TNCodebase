#!/usr/bin/env julia
# examples/models/prebuilt/heisenberg/run_dmrg.jl
#
# Heisenberg Model: Complete DMRG Ground State Search
#
# This example demonstrates:
# - Config-driven workflow (everything in config.json)
# - Automatic model building (Heisenberg using prebuilt template)
...
println("  Jx (XX coupling): $(config["model"]["params"]["Jx"])")
println("  Jy (YY coupling): $(config["model"]["params"]["Jy"])")
println("  Jz (ZZ coupling): $(config["model"]["params"]["Jz"])")
println("  hx (X field):     $(config["model"]["params"]["hx"])")
println("  hy (Y field):     $(config["model"]["params"]["hy"])")
println("  hz (Z field):     $(config["model"]["params"]["hz"])")
...
println("\n" * "="^70)
println("Success! You've run a Heisenberg DMRG simulation with TNCodebase.")
println("="^70)
