...
## Design Philosophy
- **Single function names** (`_contract_right`, `_contract_left`) with multiple dispatch
...
"""
    _contract_right(B, R, W) → Environment tensor
...
function _contract_right(B::AbstractArray{T1,3}, R::AbstractArray{T2,3}, W::AbstractArray{T3,4}) where {T1<:Number, T2<:Number, T3<:Number}
...
"""
    _contract_right(B, R, O) → Environment tensor
...
function _contract_right(B::AbstractArray{T1,3}, R::AbstractArray{T2,2}, O::AbstractArray{T3,2}) where {T1<:Number, T2<:Number, T3<:Number}
    @tensoropt fin[-1,-2] := conj(B)[-1,3,4]*R[4,5]*O[3,6]*B[-2,6,5]
    return fin
end

"""
    _contract_right(B, R) → Environment tensor
...
function _contract_right(B::AbstractArray{T1,3}, R::AbstractArray{T2,2}) where {T1<:Number, T2<:Number}
...
"""
    _contract_left(A, L, W) → Environment tensor
...
function _contract_left(A::AbstractArray{T1,3},L::AbstractArray{T2,3},W::AbstractArray{T3,4}) where {T1<:Number, T2<:Number, T3<:Number}
    @tensoropt fin[-1,-2,-3] := conj(A)[5,4,-1]*L[5,6,7]*W[6,-2,4,8]*A[7,8,-3]
    return fin
end

"""
    _contract_left(A, L, O) → Environment tensor
...
function _contract_left(A::AbstractArray{T1,3},L::AbstractArray{T2,2},O::AbstractArray{T3,2}) where {T1<:Number, T2<:Number, T3<:Number}
    @tensoropt fin[-1,-2] := conj(A)[4,3,-1]*L[4,5]*O[3,6]*A[5,6,-2]
    return fin
end

"""
    _contract_left(A, L) → Environment tensor
...
function _contract_left(A::AbstractArray{T1,3},L::AbstractArray{T2,2}) where {T1<:Number, T2<:Number}
