...
"""
function _parse_dtype(dtype_str::String)
    dtype_map = Dict(
        "Float64" => Float64,
...
"""
function _build_sites_from_config(system_config)
    system_type = system_config["type"]
    dtype = _parse_dtype(get(system_config, "dtype", "ComplexF64"))
    
    if system_type == "spin"
        return _build_spin_system(system_config, dtype)
    elseif system_type == "spinboson"
        return _build_spinboson_system(system_config, dtype)
    else
...
"""
function _build_spin_system(config, dtype)
    N = config["N"]
...
"""
function _build_spinboson_system(config, dtype)
...
"""
function _get_label_polarized(N::Int, direction::Symbol, eigenstate::Int)
...
"""
function _get_label_neel(N::Int, direction::Symbol; 
...
"""
function _get_label_kink(N::Int, direction::Symbol; position::Int=Int(ceil(N/2)),
...
"""
function _get_label_domain(N::Int, direction::Symbol; 
...
"""
function _get_label_spinboson_neel(N_spins::Int, boson_level::Int,
                                    spin_direction::Symbol;
                                    even_state::Int=1, 
                                    odd_state::Int=2)
    spin_label = _get_label_neel(N_spins, spin_direction, 
...
"""
function _get_label_spinboson_kink(N_spins::Int, boson_level::Int,
                                    direction::Symbol; 
                                    position::Int=Int(ceil(N_spins/2)),
                                    left_state::Int=1, right_state::Int=2)
    spin_pattern = _get_label_kink(N_spins, direction, position=position,
...
Boson + spin kinks
"""
function _get_label_spinboson_domain(N_spins::Int, boson_level::Int,
                                    direction::Symbol; 
                                    start_index::Int=Int(ceil(N_spins/2)),
                                    domain_size::Int=Int(ceil(N_spins/2)),
                                    base_state::Int=1, flip_state::Int=2)
    spin_pattern = _get_label_domain(N_spins, direction, 
...
"""
function _parse_spin_custom_pattern(pattern_spec)
    pattern = []
...
"""
function _parse_spinboson_custom_pattern(boson_level::Int, spin_pattern_spec)
    spin_pattern = _parse_spin_custom_pattern(spin_pattern_spec)
    return vcat(boson_level, spin_pattern)
end
...
"""
function _build_product_from_pattern(sites::Vector{<:AbstractSite}, pattern::Vector)
...
"""
function _build_random_mps(sites::Vector{<:AbstractSite}, bond_dim::Int, dtype)
...
"""
function _build_random_from_config(sites, state_config, system_config)
    params = get(state_config, "params", Dict())
    bond_dim = get(params, "bond_dim", 10)
    dtype = _parse_dtype(get(system_config, "dtype", "ComplexF64"))
    
    return _build_random_mps(sites, bond_dim, dtype)
end
...
function build_mps_from_config(config)
    ...
    if state_type == "random"
        return _build_random_from_config(sites, state_config, config["system"])
        
    elseif state_type == "prebuilt"
        if system_type == "spin"
            return _build_spin_prebuilt_state(sites, state_config)
        elseif system_type == "spinboson"
            return _build_spinboson_prebuilt_state(sites, state_config, config["system"])
        end
        
    elseif state_type == "custom"
        if system_type == "spin"
            return _build_spin_custom_state(sites, state_config)
        elseif system_type == "spinboson"
            return _build_spinboson_custom_state(sites, state_config, config["system"])
        end
...
function _build_spin_prebuilt_state(sites, config)
    name = config["name"]
    ...
    if name == "polarized"
        ...
        pattern = _get_label_polarized(N, spin_direction, eigenstate)
        
    elseif name == "neel"
        even_state = get(params, "even_state", 1)
        odd_state = get(params, "odd_state", 2)
        pattern = _get_label_neel(N, spin_direction, even_state=even_state, odd_state=odd_state)
        
    elseif name == "kink"
        position = params["position"]
        left_state = get(params, "left_state", 1)
        right_state = get(params, "right_state", 2)
        pattern = _get_label_kink(N, spin_direction, position=position,
    ...
    elseif name == "domain"
        ...
        pattern = _get_label_domain(N, spin_direction, start_index=start_index,
                                   domain_size=domain_size,
    ...
    return _build_product_from_pattern(sites, pattern)
end
...
function _build_spinboson_prebuilt_state(sites, config, system_config)
    name = config["name"]
    params = get(config, "params", Dict())
    ...
    elseif name == "neel"
        even_state = get(params, "even_state", 1)
        odd_state = get(params, "odd_state", 2)
        pattern = _get_label_spinboson_neel(N_spins, boson_level, spin_direction,
    ...
    elseif name == "kink"
        ...
        left_state = get(params, "left_state", 1)
        right_state = get(params, "right_state", 2)
        pattern = _get_label_spinboson_kink(N_spins, boson_level, spin_direction,
    ...
    elseif name == "domain"
        ...
        base_state = get(params, "base_state", 1)
        flip_state = get(params, "flip_state", 2)
        pattern = _get_label_spinboson_domain(N_spins, boson_level, spin_direction,
...
"""
function _build_spin_custom_state(sites, config)
    ...
    # Parse pattern
    pattern = _parse_spin_custom_pattern(pattern_spec)
...
"""
function _build_spinboson_custom_state(sites, config, system_config)
    ...
    # Parse pattern
    pattern = _parse_spinboson_custom_pattern(boson_level, spin_pattern_spec)
